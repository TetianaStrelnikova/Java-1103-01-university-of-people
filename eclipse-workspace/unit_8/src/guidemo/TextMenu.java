...
public class TextMenu extends JMenu {
	...
	public TextMenu(DrawPanel owner) {
		...
		lineHeight.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				String s = JOptionPane.showInputDialog(panel, "Multiply the default Line height for what amount?",currentSize);
				if (s != null && s.trim().length() > 0) {
					try {
						double newSize = Integer.parseInt(s.trim()); // can throw NumberFormatException
	...
	Creates the "Justify" menu, which contains options for text justification.

	@return The created "Justify" menu.
	*/
	...
	private JMenu makeJustifyMenu() {
	    ...
	    leftItem.addActionListener(e -> {panel.getTextItem().setJustify(TextItem.LEFT); panel.repaint();});
	    justifyMenu.add(leftItem);
	    justifyGroup.add(leftItem);

	    JRadioButtonMenuItem rightItem = new JRadioButtonMenuItem("Right");
	    rightItem.addActionListener(e -> {panel.getTextItem().setJustify(TextItem.RIGHT); panel.repaint();});
	    justifyMenu.add(rightItem);
	    justifyGroup.add(rightItem);

	    JRadioButtonMenuItem centerItem = new JRadioButtonMenuItem("Center");
	    centerItem.addActionListener(e -> {panel.getTextItem().setJustify(TextItem.CENTER); panel.repaint();});
	    justifyMenu.add(centerItem);
	    justifyGroup.add(centerItem);
	...
	public void setDefaults() {
	    ...
	    ((JRadioButtonMenuItem) justifyMenu.getMenuComponent(0)).setSelected(true); // Select the "Left" radio button
	}
