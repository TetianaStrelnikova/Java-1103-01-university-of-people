...
public class DrawTextPanel extends JPanel {
	...
	public void doMousePress(MouseEvent e) {
		...
		s.setTextColor(new Color(random.nextInt(256), random.nextInt(256), random.nextInt(256))); // Default is null, meaning default color of the canvas (black).
...
//   SOME OTHER OPTIONS THAT CAN BE APPLIED TO TEXT ITEMS:
		...
		s.setRotationAngle(random.nextDouble() * 360);//s.setRotationAngle(25); // Default is 0, meaning no rotation.
		s.setTextTransparency(random.nextDouble());
		s.setBackground(new Color(random.nextInt(256), random.nextInt(256), random.nextInt(256)));
		s.setBackgroundTransparency(random.nextDouble());
...
//		s.setBackgroundTransparency(0.7); // Default is 0, meaning background is not transparent.
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			File output = fileChooser.getOutputFile(this, "Select Image File Name", "textdata.txt");
			if (output == null)
				return;
			try (PrintWriter writer = new PrintWriter(output)) {
				// The try-with-resources statement is a feature introduced in Java 7 to
				// simplify resource management and ensure proper cleanup of resources.
				// It provides a convenient way to automatically close resources
				// that implement the AutoCloseable interface, such as file streams, database
				// connections, network sockets, and more.
				...
				// Write the text and color of each DrawTextItem
				for (DrawTextItem item : theString) {
					writer.println(item.getString());
					writer.println(item.getX());
			        writer.println(item.getY());
					Color itemColor = item.getTextColor();
					writer.println(itemColor.getRed());
					writer.println(itemColor.getGreen());
					writer.println(itemColor.getBlue());
					writer.println(item.getFont().getFontName());
					writer.println(item.getFont().getStyle());
					writer.println(item.getFont().getSize());
					writer.println(item.getMagnification());
					writer.println(item.getBorder());
					writer.println(item.getRotationAngle());
					writer.println(item.getTextTransparency());
					Color background = item.getBackground();
					if (background != null) {
	                    writer.println(background.getRed());
	                    writer.println(background.getGreen());
	                    writer.println(background.getBlue());
	                } else {
	                    writer.println("null");
	                }
					writer.println(item.getBackgroundTransparency());
					System.out.println(itemColor);
					System.out.println(itemColor.getRed());
					System.out.println(itemColor.getGreen());
					System.out.println(itemColor.getBlue());
			...
			File input = fileChooser.getInputFile(this, "Select File to Open");
			if (input == null)
				return;
			try (Scanner scanner = new Scanner(input)) {
				...
				Color backgroundColor = new Color(bgRed, bgGreen, bgBlue);
				canvas.setBackground(backgroundColor);

				// Rebuild the list of strings
				ArrayList<DrawTextItem> newStringList = new ArrayList<>();
				while (scanner.hasNextLine()) {
				    ...
				    // Read and set the additional properties
				    String fontName = scanner.nextLine();
				    int fontStyle = Integer.parseInt(scanner.nextLine());
				    int fontSize = Integer.parseInt(scanner.nextLine());
				    item.setFont(new Font(fontName, fontStyle, fontSize));
				    item.setMagnification(Double.parseDouble(scanner.nextLine()));
				    item.setBorder(Boolean.parseBoolean(scanner.nextLine()));
				    item.setRotationAngle(Double.parseDouble(scanner.nextLine()));
				    item.setTextTransparency(Double.parseDouble(scanner.nextLine()));
				    String backgroundString = scanner.nextLine();
				    if (!backgroundString.equals("null")) {
				        int backgroundRed = Integer.parseInt(backgroundString);
				        int backgroundGreen = Integer.parseInt(scanner.nextLine());
				        int backgroundBlue = Integer.parseInt(scanner.nextLine());
				        Color background = new Color(backgroundRed, backgroundGreen, backgroundBlue);
				        item.setBackground(background);
				    }
				    item.setBackgroundTransparency(Double.parseDouble(scanner.nextLine()));
				    
		            
					newStringList.add(item);
				}

				theString = newStringList;
				undoMenuItem.setEnabled(true);
				canvas.repaint();
			} catch (FileNotFoundException e) {
				JOptionPane.showMessageDialog(this, "File not found: " + e.getMessage());
			} catch (NumberFormatException e) {
				JOptionPane.showMessageDialog(this, "Invalid file format: " + e.getMessage());
			}
			...
			theString = new ArrayList<>(); // Remove the ONLY string from the canvas.
			undoMenuItem.setEnabled(false);
			canvas.repaint();
